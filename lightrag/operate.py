...
async def _merge_edges_then_upsert(
    ...
    description = await _handle_entity_relation_summary(f"({src_id}, {tgt_id})", description, global_config)
...
async def extract_entities(
    ...
    async def _user_llm_func_with_cache(input_text: str, history_messages: list[dict[str, str]] = None) -> str:
        if enable_llm_cache_for_entity_extract and llm_response_cache:
            need_to_restore = False
            if global_config["embedding_cache_config"] and global_config["embedding_cache_config"]["enabled"]:
            ...
            cached_return, _1, _2, _3 = await handle_cache(llm_response_cache, arg_hash, _prompt, "default")
            if need_to_restore:
    ...
    for result in tqdm_async(
        asyncio.as_completed([_process_single_content(c, already_processed, already_entities, already_relations, global_config, entity_types, examples, language) for c in ordered_chunks]),
    ...
    all_entities_data = []
    for result in tqdm_async(
        asyncio.as_completed([_merge_nodes_then_upsert(k, v, knowledge_graph_inst, global_config) for k, v in maybe_nodes.items()]),
    ...
    for result in tqdm_async(
        asyncio.as_completed([_merge_edges_then_upsert(k[0], k[1], v, knowledge_graph_inst, global_config) for k, v in maybe_edges.items()]),
    ...
    if relationships_vdb is not None:
        data_for_vdb = {
            compute_mdhash_id(dp["src_id"] + dp["tgt_id"], prefix="rel-"): {
                ...
                "metadata": {"created_at": dp.get("metadata", {}).get("created_at", time.time())},
            }
...
async def _get_node_data(
    ...
    node_datas = [
        {**n, "entity_name": k["entity_name"], "rank": d} for k, n, d in zip(results, node_datas, node_degrees) if n is not None
...
async def _find_most_related_text_unit_from_entities(
    ...
    all_text_units = [{"id": k, **v} for k, v in all_text_units_lookup.items() if v is not None and v.get("data") is not None and "content" in v["data"]]
...
async def _get_edge_data(
    ...
    edge_datas = await asyncio.gather(*[knowledge_graph_inst.get_edge(r["src_id"], r["tgt_id"]) for r in results])
...
async def mix_kg_vector_query(
    ...
    cached_response, quantized, min_val, max_val = await handle_cache(hashing_kv, args_hash, query, "mix")
    if cached_response is not None:
        return cached_response
    ...
    async def get_kg_context():
        try:
            ...
            kw_prompt = PROMPTS["keywords_extraction"].format(query=query, examples=examples, language=language)
            result = await use_model_func(kw_prompt, keyword_extraction=True)

            match = re.search(r"\{.*\}", result, re.DOTALL)
            if not match:
                logger.warning("No JSON-like structure found in keywords extraction result")
                return None
